...
public abstract class S7Connection {
    ...
    public ResultSet execReadRequest(final PDU p) throws IOException {
        try {
            ...
            throw new IOException("Interrupted while acquiring S7 exec read semaphore", e);
        }
        try {
            PDU p2;
            int errorState;
            ...
            if (p2.mem[p2.param + 0] == PDU.FUNC_READ) {
                ...
                for (int i = 0; i < numResults; i++) {
                    ...
                    if (r.error == 255) {
                        ...
                        log.warn("S7 read result item returned error={}", r.error);
                    }
                    pos += 4;
                    rs.results[i] = r;
                }
    ...
    public int readBytes(final DaveArea area, final int DBnum, final int start, final int len, final byte[] buffer) throws IOException {
        try {
            ...
            Thread.currentThread().interrupt();
            throw new IOException("Interrupted while acquiring S7 read semaphore area=" + area
                    + " areaNumber=" + DBnum + " offset=" + start + " length=" + len, e);
        }
        try {
            int res;
    ...
    public int writeBytes(final DaveArea area, final int DBnum, final int start, final int len, final byte[] buffer) throws IOException {
        try {
            ...
            Thread.currentThread().interrupt();
            throw new IOException("Interrupted while acquiring S7 write semaphore area=" + area
                    + " areaNumber=" + DBnum + " offset=" + start + " length=" + len, e);
        }
        try {
            int errorState;
